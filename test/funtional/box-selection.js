...
describe('box-selection redirect', function () {
  ...
  describe('direct-url does not exist', function () {
    beforeEach(function(done) {
      sinon.stub(Runnable.prototype, 'checkAndSetIfDirectUrl').yields(null, {
        statusCode: 404
      });
      sinon.stub(Runnable.prototype, 'redirectToBoxSelection', function(url, res) {
        res.redirect(301, testTargetUrl);
      });
      done();
    });
    afterEach(function(done) {
      Runnable.prototype.checkAndSetIfDirectUrl.restore();
      Runnable.prototype.redirectToBoxSelection.restore();
      done();
    });
    it('should redir to box-selection if direct-url does not exist', function (done) {
      request({
        qs: {
