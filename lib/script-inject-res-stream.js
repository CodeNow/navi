...
function createScriptInjectResStream (script, resIsGziped) {
  ...
  function injectScriptInHTMLTagFromChunk (searchString, chunk) {
    if (needToAddScript) {
      var searchStringWithPrev = prevChars + chunk;
      var indexOfSearchString = searchStringWithPrev.indexOf(searchString);
      if (indexOfSearchString > -1) {
        var endOfSearchStringInChunk = indexOfSearchString - prevChars.length + searchString.length;
        chunk = chunk.slice(0, endOfSearchStringInChunk) + script + chunk.slice(endOfSearchStringInChunk);
