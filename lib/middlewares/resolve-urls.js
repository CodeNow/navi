...
//                                              subdomain.  shortHash   -(-)      repo -staging-(anything)
var regexForRemovingShortHashFromDirectUrls = /^([^.]+\.)?(([A-z0-9]+)(-{1,2}))?(([^.]+-)+staging-.+)$/;
// regex101 on this https://regex101.com/r/3Mp4FV/1
...
module.exports.splitDirectUrlIntoShortHashAndElastic = function splitDirectUrlIntoShortHashAndElastic (directUrl) {
  ...
  if (matchingArray && matchingArray.length && matchingArray[2]) {
    // 0th is the original value (if it matched), 1st is subdomain,
    // 2nd is the shorthash and -(-), 3rd is the shorthash, 4th is the -(-) (2nd will be undefined if direct)
    // 5th is the elasticUrl
    return {
      shortHash: matchingArray[3],
      isolated: matchingArray[4] === '--',
      elasticUrl: matchingArray[5]
    };
  }
  log.error({
    directUrl: directUrl
