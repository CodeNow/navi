...
//                                                shorthash  -(-)    instance-staging-org.usercontent.domain
...
module.exports.splitDirectUrlIntoShortHashAndElastic = function splitDirectUrlIntoShortHashAndElastic (directUrl) {
  ...
  if (matchingArray && matchingArray.length) {
    // We want the 3rd and 5th item in the array
    // 1st is the original value (if it matched), 2nd is the shorthash and -(-), 4th is the -(-)
    return {
      shortHash: matchingArray[2],
      isolated: matchingArray[3] === '--',
      elasticUrl: matchingArray[4]
    };
  }
  log.error({
    directUrl: directUrl
