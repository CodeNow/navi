...
module.exports.middleware = function (req, res, next) {
  ...
  redis.lrange(redisKey, 0, 1, function (err, rawHipacheEntry) {
    if (err) {
      log.error({ err: err }, 'redis.lrange error');
      return next(err);
    }

    // if https and entry does not exist, try port 80
    if (isEmpty(rawHipacheEntry) && req.isHttps){
      log.trace('https entry not found, attempting 80');
      var redisKey80 = [ 'frontend:80.', req.parsedReqUrl.hostname ].join('');

      return redis.lrange(redisKey80, 0, 1, function (err, rawHipacheEntry) {
        if (err) {
          log.error({ err: err }, 'redis.lrange 80 error');
          return next(err);
        }
        req.reqUrl = 'http://' + req.parsedReqUrl.hostname + ':80';
        req.parsedReqUrl = url.parse(req.reqUrl);
        module.exports.getMongoEntry(req, rawHipacheEntry, next);
      });
    }
...
module.exports.getMongoEntry = function (req, rawHipacheEntry, next) {
  var log = logger.child({
    tx: true,
    req: req,
  ...
  if (isEmpty(rawHipacheEntry)) {
    log.error('no hipache entry found!');
    return next(new Error('no hipache entry found'));
  }

  try {
    req.hipacheEntry = JSON.parse(rawHipacheEntry[0]);
  } catch (err) {
    log.error({ err: err }, 'json parse error');
    return next(err);
  }
  ...
  mongo.fetchNaviEntry(elasticUrl, req.refererUrlHostname, function (err, naviEntry) {
    ...
    monitor.increment('navi.url-resolved.count', {
      isBrowser: req.isBrowser,
      orgName: naviEntry.ownerUsername,
      org: naviEntry.ownerGithubId
    });
    return next();
  });
};
