...
module.exports.getMongoEntry = function (req, next) {
  ...
  mongo.fetchNaviEntry(givenUrl, req.refererUrlHostname, function (err, naviEntry) {
    ...
    if (naviEntry.ipWhitelist && naviEntry.ipWhitelist.enabled === true && !naviEntry.ipWhitelist.ips.includes(req.hostIp)) {
      log.info('External attempt to access ip restricted URLs.')
      return next(new RouteError('Forbidden', 403));
    }
