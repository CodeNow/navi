...
module.exports.middleware = function (req, res, next) {
  ...
  if (req.refererUrl) {
    ...
    if (req.parsedRefererUrl.host) {
      const hostParts = req.parsedRefererUrl.host.split('.');
      if (hostParts.length > 2) {
        log.trace({ host: req.parsedRefererUrl.host}, 'URL has more than 2 dots in its host');
        req.parsedRefererUrl.host = `${hostParts[hostParts.length - 3]}.${hostParts[hostParts.length - 2]}.${hostParts[hostParts.length - 1]}`;
        req.refererUrl = url.format(req.parsedRefererUrl);
        req.parsedRefererUrl = url.parse(req.refererUrl);
      }
    }
