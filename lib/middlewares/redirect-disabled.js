 ...
 */
module.exports = function redirectDisabledMiddleware(req, res, next) {
  var log = logger.child({ tx: true, req: req });
  if (req.naviEntry.redirectEnabled === true) {
    return next();
  }
  log.info('middlewares/redirect-disabled');
...
module.exports._makeDecision = function _makeDecision(req, res, next) {
  ...
  log.info('middlewares/redirect-disabled _makeDecision');
...
module.exports._getMasterBranchContainer = function _getMasterBranchContainer(req) {
  var log = logger.child({ tx: true, req: req });
  log.info('middlewares/redirect-disabled _getMasterBranchContainer');
...
module.exports._getConnectionContainer = function _getConnectionContainer(req) {
  var log = logger.child({ tx: true, req: req });
  log.info('middlewares/redirect-disabled _getConnectionContainer');
...
module.exports._getRequestedContainer = function _getRequestedContainer(req) {
  var log = logger.child({ tx: true, req: req });
  log.info('middlewares/redirect-disabled _getRequestedContainer');
...
module.exports._proxyRequest = function _proxyRequest(container, req, res, next) {
  var log = logger.child({ tx: true, req: req, targetNaviEntryInstance: container });
  log.info('middlewares/redirect-disabled _proxyRequest');
  if (!container) {
