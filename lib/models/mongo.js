...
Mongo.prototype.fetchNaviEntry = function (elasticUrl, refererUrl, cb) {
  ...
  /**
   * Helper function, either invoked as a mongo callback or directly if mongo results are cached
   * @param {Boolean} cacheResults
   * @param {Object|Null} err
   * @param {Array} response
   */
  function handleData (cacheResults, err, response) {
    if (err) {
      log.error(put({
        err: err
      }, logData), 'fetchNaviEntry handleData error');
      return cb(err);
    }
    assign(logData, {
      response: response
    });
    log.trace(logData, 'fetchNaviEntry handleData success');

    var naviEntry;
    if (response.length === 1) {
      log.trace(logData, 'fetchNaviEntry handleDatalength 1');
      naviEntry = response[0];
    } else if (response.length === 2) {
      log.trace(logData, 'fetchNaviEntry handleData length 2');
      ...
      log.error(logData, 'fetchNaviEntry handleData invalid length');
      return cb(ErrorCat.create(500, 'internal server error'));
    }

    if (cacheResults) {
      log.trace(logData, 'fetchNaviEntry handleData cache')
      self._cacheResults(naviEntry);
    }
  ...
  var cachedData = this._getCachedData(elasticUrl, refererUrl);
  if (cachedData) {
    // Arguments: {Boolean} cache, {Object} err, {Array} <NaviEntry>
    handleData.apply(this, false, null, cachedData);
  } else {
    this._naviEntriesCollection.find(query).toArray(handleData.bind(this, true));
  }
};
 ...
 */
Mongo.prototype._getCacheResults = function (elasticUrl, refererUrl) {
  var logData = {
    tx: true,
    elasticUrl: elasticUrl,
    refererUrl: refererUrl
  ...
  if (refererUrl) {
    ...
    if (elasticUrlCache && refererUrlCache) {
      return handleData(false, null, [elasticUrlCache, refererUrlCache]);
    }
  } else {
    // no referer, just check if we have the elastic cached
    if (elasticUrlCache) {
      return handleData(false, null, [elasticUrlCache]);
    }
  }
...
Mongo.prototype._cacheResults = function (naviEntry) {
  if (naviEntry.refererNaviEntry) {
    cache.set(naviEntry.refererNaviEntry.elasticUrl, put({}, naviEntry.refererNaviEntry));
    var copyNaviEntry = put({}, naviEntry);
    ...
    cache.set(naviEntry.elasticUrl, put({}, naviEntry));
  }
};
