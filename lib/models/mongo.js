 ...
 * @param {String} elasticHostname main url's hostname
 * @param {Function} cb
 */
Mongo.prototype.fetchNaviEntry = function (elasticHostname, refererUrl, cb) {
  var logData = {
    tx: true,
  ...
  if (refererUrl) {
    ...
    query.elasticUrl = elasticHostname;
  }
  ...
  var cachedData = this._getCachedResults(elasticHostname, refererUrl);
  ...
  if (cachedData) {
    ...
    this._fetchNaviEntryHandleCacheOrMongo(false, null, cachedData, elasticHostname, cb);
  } else {
    log.trace(logData, 'fetchNaviEntry !cachedData');
    monitor.increment('navi.cache.miss')
    this._naviEntriesCollection.find(query).toArray(function (err, response) {
      self._fetchNaviEntryHandleCacheOrMongo.call(self, true, err, response, elasticHostname, cb);
    });
  }
};
