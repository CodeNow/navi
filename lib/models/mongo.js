  ...
  if (refererUrl) {
    query.$and.push({
      $or: [{
        ...
        elasticUrl: resolveUrls.splitDirectUrlIntoShortHashAndElastic(elasticUrl).elasticUrl
      }, {
        elasticUrl: refererUrl
      }, {
        elasticUrl: resolveUrls.splitDirectUrlIntoShortHashAndElastic(refererUrl).elasticUrl
      }]
    });
  } else {
    query.$and.push({
...
Mongo.prototype._fetchNaviEntryHandleCacheOrMongo = function (
  ...
  var naviEntry;
  // Hopefully response never has more than 2
  if (response.length === 1 || (response.length === 2 && response[0].elasticUrl === response[1].elasticUrl) ) {
    /**
