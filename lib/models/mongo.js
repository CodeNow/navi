...
var regexForDirectUrlPrefix = /^[A-z0-9]*-{1,2}/
...
Mongo.prototype.fetchNaviEntry = function (elasticUrl, refererUrl, cb) {
  ...
  function convertDirectToElastic (directUrl) {
    return directUrl.replace(regexForDirectUrlPrefix, '')
  }
  if (refererUrl) {
    query.$and.push({
      $or: [{
        ...
        elasticUrl: convertDirectToElastic(refererUrl)
      }]
    });
  } else {
    query.$and.push({
