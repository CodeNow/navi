  ...
  function handleData (err, response) {
    ...
    if (response.length === 1) {
      ...
      if (response[0].elasticUrl === elasticUrl) {
        naviEntry = response[0];
        cache.set(elasticUrl, put({}, naviEntry)); // don't cache w/ attached refererNaviEntry
        naviEntry.refererNaviEntry = response[1];
        cache.set(refererUrl, put({}, naviEntry.refererNaviEntry));
      } else {
  ...
  if (process.env.ENABLE_LRU_CACHE) {
    ...
    if (refererUrl) {
      ...
      if (elasticUrlCache && refererUrlCache) {
        return handleData(null, [elasticUrlCache, refererUrlCache]);
      }
    } else {
      // no referer, just check if we have the elastic cached
      if (elasticUrlCache) {
        return handleData(null, [elasticUrlCache]);
      }
    }
  }

  this._naviEntriesCollection.find(query).toArray(handleData);
};
