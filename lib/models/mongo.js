...
var Experiment = require('lru-experiment');
...
Mongo.prototype.fetchNaviEntry = function (elasticUrl, refererUrl, cb) {
  ...
  /**
   * Test of using node-experiment to "dark launch" or test LRU cache at our current level of
   * production scale. To be removed after production test
   */
  var experiment = new Experiment();

  // Candidate
  experiment.try(function () {
    log.info(logData, 'fetchNaviEntry Navi-LRU-cache experiment try');
    return withLRUCache();
  });

  // Control
  experiment.use(function () {
    log.info(logData, 'fetchNaviEntry Navi-LRU-cache experiment use');
    return withoutLRUCache();
  });

  /**
   * Candidate
   *
   * Will check if data exists in LRU cache. If data is found in cache, request will continue using
   * cached data. If data is not found, it will be fetched from Mongo.
   */
  function withLRUCache () {
    log.info(logData, 'fetchNaviEntry Navi-LRU-cache experiment withLRUCache');
    return Promise.fromCallback(function (promiseCb) {
      var cachedData = self._getCachedResults(elasticUrl, refererUrl);
      if (cachedData) {
        log.trace(logData, 'fetchNaviEntry cachedData');
        // Arguments: {Boolean} shouldCacheResults, {Object} err, {Array} <NaviEntry>, cb
        self._fetchNaviEntryHandleCacheOrMongo(false, null, cachedData, elasticUrl, promiseCb);
      } else {
        log.trace(logData, 'fetchNaviEntry !cachedData');
        self._naviEntriesCollection.find(query).toArray(function (err, response) {
          self._fetchNaviEntryHandleCacheOrMongo
            .call(self, true, err, response, elasticUrl, promiseCb);
        });
      }
    });
  }
  ...
  /**
   * Control
   * Original logic. Fetches navi-entry documents from MongoDB for each request with no caching.
   *
   * withoutLRUCache will next() the request; the request will not be blocked until both the
   * candidate and the control resolve.
   */
  function withoutLRUCache () {
    log.info(logData, 'fetchNaviEntry Navi-LRU-cache experiment withoutLRUCache');
    return Promise.fromCallback(function (promiseCb) {
      self._naviEntriesCollection.find(query).toArray(function (err, response) {
        self._fetchNaviEntryHandleCacheOrMongo
          .call(self, true, err, response, elasticUrl, function () {
          log.trace(logData, 'fetchNaviEntry Navi-LRU-cache experiment withoutLRUCache - complete');
          promiseCb.apply(this, arguments);
          // Continue request
  ...
  experiment.run().asCallback(function (err, results) {
    log.info(logData, 'Navi-LRU-cache experiment run() asCallback')
  });

  /*
  // TODO: Restore this and remove experiment after testing
...
Mongo.prototype._cacheResults = function (naviEntry) {
  ...
  log.info(logData, '_cacheResults !ENABLE_LRU_CACHE');
  if(!process.env.ENABLE_LRU_CACHE) {
