...
Mongo.prototype.fetchNaviEntry = function (elasticUrl, refererUrl, cb) {
   ...
   */
  function handleData (shouldCacheResults, err, response) {
    if (err) {
      log.error(put({
        err: err
  ...
  if (cachedData) {
    ...
    // Arguments: {Boolean} shouldCacheResults, {Object} err, {Array} <NaviEntry>
    handleData.call(this, false, null, cachedData);
  } else {
    log.trace(logData, 'fetchNaviEntry !cachedData');
...
Mongo.prototype._cacheResults = function (naviEntry) {
  ...
  if (naviEntry.refererNaviEntry) {
    var copyRefererNaviEntry = put({}, naviEntry.refererNaviEntry);
