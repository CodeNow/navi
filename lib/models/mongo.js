...
function (shouldCacheResults, err, response, elasticUrl, cb) {
  ...
  if (err) {
    log.error(put({
      err: err
    }, logData), '_fetchNaviEntryHandleCacheOrMongo handleData error');
    return cb(err);
  }
  ...
  log.trace(logData, '_fetchNaviEntryHandleCacheOrMongo handleData success');

  var naviEntry;
  if (response.length === 1) {
    log.trace(logData, '_fetchNaviEntryHandleCacheOrMongo handleDatalength 1');
    naviEntry = response[0];
  } else if (response.length === 2) {
    log.trace(logData, '_fetchNaviEntryHandleCacheOrMongo handleData length 2');
    ...
    log.error(logData, '_fetchNaviEntryHandleCacheOrMongo handleData invalid length');
    return cb(ErrorCat.create(500, 'internal server error'));
  }

  if (shouldCacheResults) {
    log.trace(logData, '_fetchNaviEntryHandleCacheOrMongo handleData cache')
