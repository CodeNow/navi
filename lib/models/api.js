  ...
  mongo.fetchNaviEntry(parsedReqUrl.hostname, refererUrlHostname, function (err, naviEntry) {
    ...
    if (req.isBrowser) {
      ...
      if (refererUrl && naviEntry.refererNaviEntry) {
        ...
        return api._getTargetHostElasticReferer(logData, refererUrl, naviEntry, req, next);
      } else {
        // if no referer
 ...
 * @param {Object} logData
 */
api._getTargetHostElasticReferer = function (logData, refererUrl, naviEntry, req, next) {
  // if referer
