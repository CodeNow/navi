...
api._getGithubAuthUrl = function (reqUrl, authTried) {
  ...
  return [process.env.API_HOST, '/auth/github?', query];
};
...
api._getDestinationProxyUrl = function (reqUrl, naviEntryInstance) {
  ...
  return [reqUrl.protocol, '//', destinationHost, ':', destinationPort].join('');
};
    ...
    if (info.elastic) {
      ...
      mongo.fetchNaviEntry(reqUrl, function (err, naviEntry) {
        if (req.isBrowser) {
          ...
          var naviEntryInstance = mongo.findNaviEntryShortHashByBranch('master', naviEntry);
