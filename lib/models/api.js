...
api._shouldBypassAuth = function (req) {
  if (process.env.ALLOW_UNAUTHED_PUBLIC_REQUESTS) {
    return true;
  }
  ...
  if (req.isBrowser) {
    ...
    if (refererUrl && naviEntry.refererNaviEntry) {
      ...
      log.trace(put({
        ...
        shortHash: mappedInstanceShortHash,
        session: req.session
      }, logData), '_getTargetHostElastic fetch usermapping');
    ...
    var findResult1 = mongo.constructor.findMasterPodBranch(naviEntry);
    targetNaviEntryInstance = findResult1.directUrlObj;
    return api._processTargetInstance(targetNaviEntryInstance,
                                      findResult1.directUrlShortHash,
                                      reqUrl, req, next);
  }
};
...
api._getTargetHostElasticReferer = function (logData, naviEntry, req, next) {
  ...
  if (instanceShortHash) {
     ...
     */
    var findResult2 = mongo.constructor.findMasterPodBranch(naviEntry.refererNaviEntry);
    targetNaviEntryInstance = keypather.get(findResult2, 'directUrlObj');
  }
  ...
  if (!targetNaviEntryInstance) {
    ...
    var findResult3 = mongo.constructor.findMasterPodBranch(naviEntry);
    targetNaviEntryInstance = findResult3.directUrlObj;
    return api._processTargetInstance(targetNaviEntryInstance,
                                      findResult3.directUrlShortHash,
                                      reqUrl, req, next);
...
api.getTargetHost = function (req, res, next) {
  ...
  if (!api._shouldBypassAuth(req) &&
    ...
    log.warn(logData, 'getTargetHost info.ownerGithub not in userOrgs');
    return next(ErrorCat.create(404, 'Not Found'));
  }
