 ...
 * @param {Object}   req
 * @param isAuthorized {Boolean} True if the user is authorized
 * @param {Function} next
 */
api._getTargetHostElastic = function (logData, req, isAuthorized, next) {
  ...
  mongo.fetchNaviEntry(parsedReqUrl.hostname, refererUrlHostname, function (err, naviEntry) {
    ...
    if (req.isBrowser) {
      ...
      if (!isAuthorized && keypather.get(naviEntry, 'ipWhitelist.enabled')) {
...
api.getTargetHost = function (req, res, next) {
  ...
  redis.lrange(redisKey, 0, 1, function (err, response) {
    ...
    // verification
    var isAuthorized = api._shouldBypassAuth(req) || api._isUserAuthorized(req, info.ownerGithub);
    if (!isAuthorized) {
      log.warn(put({
        session: req.session,
        info: info
    ...
    if (info.elastic) {
      ...
      return api._getTargetHostElastic(logData, req, isAuthorized, next);
    } else {
      if (!isAuthorized) {
        return next(ErrorCat.create(404, 'Not Found'));
      }
