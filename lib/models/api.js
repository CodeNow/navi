...
api.checkIfLoggedIn = function (req, res, next) {
   ...
   */
  api.getReqInfo(req, res, function (err) {
    if (err) {
      return next(err);
    }
    ...
    mongo.fetchNaviEntry(req.parsedReqUrl.hostname, req.refererUrlHostname, function (err, naviEntry) {
      if (err) {
        log.error(put({
          err: err
        }, logData), '_getTargetHostElastic redis.lrange mongo.fetchNaviEntry error');
      ...
      req.ipWhitelist = naviEntry.ipWhitelist;

      if (keypather.get(req, 'ipWhitelist.enabled')) {
        // if whitelisting is enabled
        if (!hasKeypaths(req.session, ['userId', 'apiSessionRedisKey', 'userGithubOrgs']) ||
            api._isUserAuthorized(req, req.redisInfo.ownerGithub)) {
...
api.getReqInfo = function (req, res, next) {
  /*jshint maxdepth:4*/
  var reqUrl = api._getUrlFromRequest(req);
  var parsedReqUrl = url.parse(reqUrl);
  var refererUrl = req.headers.origin || req.headers.referer;
  ...
  log.info(logData, 'api.getReqInfo');

  if (refererUrl) {
    parsedRefererUrl = url.parse(refererUrl);
    req.refererUrlHostname = parsedRefererUrl.hostname;
  }

  if (req.refererUrlHostname === parsedReqUrl.hostname) {
    log.trace(logData, 'getReqInfo refererUrlHostanme === parsedReqUrl.hostname');
    // referer is self, ignore it
    req.refererUrlHostname = null;
  }
  /**
  ...
  redis.lrange(redisKey, 0, 1, function (err, response) {
    if (err) {
      log.error(put({
        err: err
      }, logData), 'getReqInfo redis.lrange error');
      return next(err);
    }
    if (!response.length) {
      log.error(put({
        err: err
      }, logData), 'getReqInfo redis.response !length');
      return next(ErrorCat.create(400, 'No redis results'));
    }
    var info = response[0];

    if (info.direct) {
      var splitReqHostname = parsedReqUrl.hostname.split('-');
      req.shortHash = splitReqHostname.shift();
      req.elasticUrl = splitReqHostname.join('-');
    }
    assign(logData, {
      info: info
    });
...
api.getTargetHost = function (req, res, next) {
  ...
  if (info.elastic) {
    ...
    var shortHash = req.shortHash;
    var elasticUrl = req.elasticUrl;
    assign(logData, {
      shortHash: shortHash,
