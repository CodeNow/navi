    ...
    urlEntry.getInfo(function (err, info) {
      ...
      var instances = client.fetchInstances({
        ...
        if (err) {
          log.error(put({
            err: err
          }, checkUrlLogData), 'api.getTargetHost checkUrl urlEntry.getInfo client.fetchInstances error');
          return next(err);
        }
        var instance = instances.models[0];
        if (!instance) {
          log.warn(checkUrlLogData, 'api.getTargetHost checkUrl urlEntry.getInfo client.fetchInstances !instance');
        ...
        if (info.elastic) {
          ...
          log.trace(checkUrlLogData, 'api.getTargetHost checkUrl urlEntry.getInfo client.fetchInstances info.elastic');
          api._handleElasticUrl(client, reqUrl, refererUrl, instance, proxyToContainer);
        }
        else { // direct url
          if (!req.isBrowser) {
            log.trace(checkUrlLogData, 'api.getTargetHost checkUrl urlEntry.getInfo client.fetchInstances !req.isBrowser');
...
api._handleElasticUrl = function (client, elasticUrl, refererUrl, masterInstance, cb) {
  ...
  function checkReferer (cb) {
    var checkRefererLogData = put({}, logData);
    log.info(checkRefererLogData, 'api._handleElasticUrl checkReferer');
    if (!refererUrl) {
      log.trace(checkRefererLogData, 'api._handleElasticUrl checkReferer !refererUrl');
      return checkForMapping(boxSelectionUrl, cb);
    }
    var refererHostname = url.parse(refererUrl).hostname;
    if (refererHostname === 'localhost') {
      log.trace(checkRefererLogData, 'api._handleElasticUrl checkReferer localhost');
      ...
      log.trace(checkRefererLogData, 'api._handleElasticUrl checkReferer refererHostname === elasticHostname');
      return checkForMapping(masterUrl, cb);
    }
    ...
    refererEntry.exists(function (err, entryExists) {
      if (err) {
        log.error(put({
          err: err
        }, checkRefererLogData), 'api._handleElasticUrl checkReferer refererEntry.exists error');
        return cb(err);
      }
      if (entryExists) {
        log.trace(put({
          entryExists: entryExists
        }, checkRefererLogData), 'api._handleElasticUrl checkReferer refererEntry.exists entryExists');
  ...
  function checkForAssociations(refererHostname, cb) {
    ...
    findUserMapping(refererHostname, function (err, refererRoute) {
      if (err) {
        log.error(put({
          err: err
        }, checkForAssociationsLogData), 'api._handleElasticUrl checkForAssociations findUserMapping error');
        return cb(err);
      }
      ...
      if (!refererRoute) {
        log.trace(checkForAssociationsLogData, 'api._handleElasticUrl checkForAssociations findUserMapping !refererRoute');
        return checkForMapping(masterUrl, cb);
      }
      log.trace(checkForAssociationsLogData, 'api._handleElasticUrl checkForAssociations findUserMapping success');
      client
        ...
        .fetchDependencies({
          ...
          if (err) {
            log.error(put({
              err: err
            }, checkForAssociationsLogData), 'api._handleElasticUrl checkForAssociations findUserMapping fetchDependencies error');
            return cb(err);
          }
          checkForAssociationsLogData = put({
          ...
          if (assoc) {
            log.trace(checkForAssociationsLogData, 'api._handleElasticUrl checkForAssociations findUserMapping fetchDependencies association found');
            var assocInstance = client.fetchInstance(assoc.id, function (err) {
              if (err) {
                log.error(put({
                  err: err
                }, checkForAssociationsLogData), 'api._handleElasticUrl checkForAssociations findUserMapping fetchDependencies fetchInstance error');
              }
              else {
                log.trace(checkForAssociationsLogData, 'api._handleElasticUrl checkForAssociations findUserMapping fetchDependencies fetchInstance success');
              }
              handleInstance(err, assocInstance, parsedElasticUrl.port, cb);
            });
          }
          else {
            log.warn(checkForAssociationsLogData, 'api._handleElasticUrl checkForAssociations findUserMapping fetchDependencies no association found');
