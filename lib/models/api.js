...
api.getTargetHost = function (req, res, next) {
   ...
   */
  redis.lrange('frontend:'+reqUrl, 0, 1, function (err, response) {
    if (err) {
      log.error(put({
        err: err
    ...
    // 1. if not found, this isn't a valid elastic or direct url
    // 3. if found, check ports in redis JSON value. See if requested port is exposed
    // This shouldn't happen, hipache only forwards requests to navi if the request is made on
    // a port actually exposed by the container
    ...
    if (info.elastic) {
      mongo.fetchNaviEntry(reqUrl, refererUrl, function (err, naviEntry) {
        if (err) {
          log.error(put({
            err: err
          }, logData), 'getTargetHost mongo.fetchNaviEntry error');
          return next(err);
        }
        if (req.isBrowser) {
          ...
          if (refererUrl) {
            ...
            // 2.
            var refererNaviEntryInstance = mongo.constructor.findUserMappedInstance(
              req, naviEntry.refererNaviEntry);
            // 3.
            targetNaviEntryInstance =
              naviEntry.directUrls[refererNaviEntryInstance.associations[reqUrl]];
          ...
          if (!naviEntryInstance) { /* ? no master instance */ }
          if (naviEntryInstance.status !== 'running') {
            req.targetHost = errorPage.generateErrorUrl('not_running', {
              elasticUrl: reqUrl,
              targetBranch: naviEntryInstance.branch
            });
            return next();
          }
