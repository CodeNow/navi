...
api.createClient = function (req, res, next) {
  ...
  var cookie = keypath.get(req, 'session.apiCookie');
  debug('cookie', cookie);
  if (cookie) {
 ...
 * sets req.targetHost if host found
 * sets req.redirectUrl if redirect needed
...
api.getTargetHost = function (req, res, next) {
  ...
  function checkUrl () {
    api._getInstancesAndNaviEntryForUrl(client, reqUrl, function (err, instances, urlEntry) {
      if (err) { return next(err); }
  ...
  function redirectToElastic (err, hostname) {
    ...
    delete parsed.host;
    req.redirectUrl = url.format(parsed);
  }
...
/**
 * finds master instance if elastic url, or normal instance if direct url
 * also creates urlEntry
 * @param  {object}   client api client
 * @param  {[type]}   reqUrl url to find instance for
 *                           <protocol>/<host>:<port>
 * @param  {Function} cb     (err, instances, urlEntry)
 *                           instances: array
 *                           urlEntry: object
 */
api._getInstancesAndNaviEntryForUrl = function (client, reqUrl, cb) {
  // reqUrl entry backendUrl is navi, no need to check existence.
  var urlEntry = NaviEntry.createFromUrl(reqUrl);
  debug('urlEntry', urlEntry);
  urlEntry.getInstanceName(function (err, instanceName) {
    ...
    var instances = client.fetchInstances({
      ...
      if (err) { return cb(err); }
      cb(null, instances, urlEntry);
    });
  });
};
/**
 * finds container url based on url provided
 * if elastic, returns url from master instance
 * if direct, returns url that instance
 * req.targetHost set if found
 * @param  {object}   req  express request
 * @param  {object}   res  express response
 * @param  {Function} next express next
 */
api.getContainerUrlForRequest = function (req, res, next) {
  var reqUrl = api._getUrlFromRequest(req);
  var client = req.apiClient;
  debug('reqUrl', reqUrl);
  api._getInstancesAndNaviEntryForUrl(client, reqUrl, function (err, instances) {
    if (err) { return next(err); }
    var instance = instances.models[0];
    if (!instance) {
      return next(ErrorCat.create(404, 'mapped instance no longer exists'));
    }
    client.newInstance(instance).getContainerUrl(function (err, containerUrl) {
      debug('targetHost', containerUrl);
      if (err) { return next(err); }
      req.targetHost = containerUrl;
      next();
    });
