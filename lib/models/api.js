...
api.getTargetHost = function (req, res, next) {
  ...
  redis.lrange(redisKey, 0, 1, function (err, response) {
    ...
    // verification
    if (!api._shouldBypassAuth(req) &&
        !~req.session.userGithubOrgs.indexOf(info.ownerGithub.toString())) {
      log.warn(put({
        session: req.session,
        info: info
    ...
    if (info.elastic) {
      ...
      mongo.fetchNaviEntry(parsedReqUrl.hostname, refererUrlHostname, function (err, naviEntry) {
        if (err) {
          log.error(put({
            err: err
          }, logData), 'getTargetHost redis.lrange mongo.fetchNaviEntry error');
          return next(err);
        }
        log.trace(logData, 'getTargetHost redis.lrange mongo.fetchNaviEntry success');
        ...
        if (req.isBrowser) {
          ...
          if (refererUrl) {
            ...
            log.trace(logData,
                      'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser refererUrl');
            ...
            // 2.
            var refererUserMappedInstanceId =
              naviEntry.refererNaviEntry.userMappings[req.session.userId];
            ...
            if (!refererUserMappedInstanceId) {
              ...
              refererNaviEntryInstance =
                mongo.constructor.findBranch('master', naviEntry.refererNaviEntry);
              log.trace(put({
                refererUserMappedInstanceId: refererUserMappedInstanceId,
                refererNaviEntryInstance: refererNaviEntryInstance
              }, logData),
              'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser '+
              '!refererNaviEntryInstance');
            } else {
              refererNaviEntryInstance =
            ...
            targetNaviEntryInstance =
              naviEntry.directUrls[refererNaviEntryInstance.associations[parsedReqUrl.hostname]];
            if (!targetNaviEntryInstance) {
              // use master
              log.trace(put({
                targetNaviEntryInstance: targetNaviEntryInstance
              }, logData),
              'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser !targetNaviEntryInstance '+
              'use master');
              targetNaviEntryInstance = mongo.constructor.findBranch('master', naviEntry);
              if (!targetNaviEntryInstance) { /* ? no master instance */ }
            }

            if (!targetNaviEntryInstance.running) {
              log.trace(put({
                targetNaviEntryInstance: targetNaviEntryInstance
              }, logData), 'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser !running');
            ...
            'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser refererUrl final');
            ...
            log.trace(logData,
                      'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser !refererUrl');
            // TODO: userId
            var mappedInstanceId = naviEntry.userMappings[req.session.userId];
            if (!mappedInstanceId) {
              log.trace(logData,
                        'getTargetHost redis.lrange mongo.fetchNaviEntry isBrowser !refererUrl '+
                        '!mappedInstanceId');
            ...
            if (!targetNaviEntryInstance.running) {
              ...
              'getTargetHost redis.lrange mongo.fetchNaviEntry !isBrowser !referer !running');
              req.targetHost = errorPage.generateErrorUrl('not_running', {
                elasticUrl: reqUrl,
                targetBranch: targetNaviEntryInstance.branch
              });
          ...
          log.trace(put({
            targetNaviEntryInstance: targetNaviEntryInstance
          }, logData), 'getTargetHost redis.lrange mongo.fetchNaviEntry !isBrowser final');
          req.targetHost = api._getDestinationProxyUrl(reqUrl, targetNaviEntryInstance);
          return next();
        }
      });
