...
api.createClient = function (req, res, next) {
  ...
  if (req.method.toLowerCase() ===  'options' || !req.isBrowser) {
    ...
    debug((!req.isBrowser ? 'non-browser' : 'options') + 'request, using super user');
    return next();
  }
    ...
    urlEntry.getInfo(function (err, info) {
      ...
      var instances = client.fetchInstances({
        ...
        if (info.elastic) {
          ...
          if (!req.isBrowser) {
            debug('non-browser direct url request, proxy to direct container');
