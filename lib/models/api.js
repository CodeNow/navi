...
api.getUrlFromRequest = function (req) {
  ...
  log.info(logData, 'api.getUrlFromRequest');
  ...
  }, logData), 'api.getUrlFromRequest final return');
 ...
 */
api.getTargetUrl = function (isHttps, parsedReqUrl, targetNaviEntryInstance) {
  ...
  log.info('called');
  var proto = parsedReqUrl.protocol;
  var port = targetNaviEntryInstance.ports[parsedReqUrl.port];

  if (isHttps &&
      !targetNaviEntryInstance.ports['443'] &&
      targetNaviEntryInstance.ports['80']) {
    log.trace('proxy https to http on port 80');
    port = targetNaviEntryInstance.ports['80'];
    proto = 'http:';
  }

  var targetUrl =  [
    proto, '//', targetNaviEntryInstance.dockerHost, ':', port
  ].join('');
...
api._processTargetInstance = function (targetNaviEntryInstance, shortHash, reqUrl, req, next) {
  ...
  req.targetHost = api.getDestinationProxyUrl(
    req.isHttps,
    req.parsedReqUrl,
