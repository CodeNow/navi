  ...
  mongo.fetchNaviEntry(parsedReqUrl.hostname, refererUrlHostname, function (err, naviEntry) {
    ...
    if (req.isBrowser) {
      ...
      if (refererUrl && naviEntry.refererNaviEntry &&
          !keypather.get(req, 'session.directShortHash[' + parsedReqUrl.hostname + ']')) {
        log.trace(logData,
        ...
        var mappedInstanceShortHash;
        if (req.session.userId) {
          mappedInstanceShortHash = (naviEntry.userMappings) ?
            naviEntry.userMappings[req.session.userId] : null;
        } else if (keypather.get(req, 'session.directShortHash')){
          mappedInstanceShortHash = req.session.directShortHash[parsedReqUrl.hostname]
        }
...
api._getTargetHostElasticReferer = function (logData, naviEntry, req, next) {
  ...
  if (req.session.userId) {
    refererUserMappedInstanceId = (naviEntry.refererNaviEntry.userMappings) ?
      naviEntry.refererNaviEntry.userMappings[req.session.userId] : null;
  } else if (keypather.get(req, 'session.directShortHash')){
    refererUserMappedInstanceId = req.session.directShortHash[naviEntry.refererNaviEntry.elasticUrl]
  }
