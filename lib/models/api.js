...
var isElasticUrl = function (reqUrl, instance) {
  var parsedReqUrl = url.parse(reqUrl);
  var masterPod = instance.attrs.masterPod;
  if (!masterPod) {
    return false;
  }
  var instanceEntry = new NaviEntry({
    exposedPort: parsedReqUrl.port,
    branch: instance.getBranchName(),
    instanceName: instance.attrs.name,
    masterPod: masterPod,
    ownerUsername: instance.attrs.owner.username,
    // this is not used, but is required to initialize naviEntry
    userContentDomain: 'doesnotmatter.com'
  });
  return (parsedReqUrl.hostname.toLowerCase().split('.')[0] !==
      instanceEntry.getDirectHostname().toLowerCase().split('.')[0]);
};
  ...
  function checkUrl () {
    ...
    urlEntry.getInstanceName(function (err, instanceName) {
      ...
      var instances = client.fetchInstances({
        ...
        if (isElasticUrl(reqUrl, instance)) {
...
api._handleElasticUrl = function (client, elasticUrl, refererUrl, masterInstance, cb) {
  ...
  function handleInstance (err, instance) {
    if (err) { return cb(err); }
    var containerUrl = instance.getContainerUrl();
