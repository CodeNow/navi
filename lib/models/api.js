  ...
  mongo.fetchNaviEntry(parsedReqUrl.hostname, refererUrlHostname, function (err, naviEntry) {
    ...
    if (req.isBrowser) {
      ...
      if (refererUrl && naviEntry.refererNaviEntry) {
        ...
        var mappedInstanceShortHash = null;
        if (req.session.userId && naviEntry.userMappings){
          mappedInstanceShortHash = naviEntry.userMappings[req.session.userId];
        } else {
          mappedInstanceShortHash = (keypather.get(req, 'session.directShortHash')) ?
...
api._getTargetHostElasticReferer = function (logData, naviEntry, req, next) {
  ...
  if (req.session.userId && naviEntry.refererNaviEntry.userMappings){
    refererUserMappedInstanceId = naviEntry.refererNaviEntry.userMappings[req.session.userId];
  } else {
    refererUserMappedInstanceId = (keypather.get(req, 'session.directShortHash')) ?
...
api._isUserAuthorized = function (req, ownerGithub) {
  if (process.env.PUBLIC_ALLOWS_UNAUTH) {
    return true
  }
...
api.getTargetHost = function (req, res, next) {
  ...
  redis.lrange(redisKey, 0, 1, function (err, response) {
    ...
    if (info.elastic) {
      ...
      if (!keypather.get(req, 'session.userId')) {
        log.trace(put({
          redirectUrl: newRedirectUrl,
          shortHash: shortHash
        }, logData), 'getTargetHost anonymous username');
