...
NaviEntry.setRedisClient(redis);
...
/**
 * Should allow request to bypass authentication checks
 * @param {Object} req
 * @return {Boolean}
 */
api._allowAuthBypass = function (req) {
  return req.method.toLowerCase() === 'options' || !req.isBrowser;
};
...
api.createClient = function (req, res, next) {
  ...
  if(api._allowAuthBypass(req)) {
    log.trace(logData, 'api.createClient session check bypass (options request or not browser)');
...
api.getTargetHost = function (req, res, next) {
  ...
  function checkUrl () {
    ...
    urlEntry.getInfo(function (err, info) {
      ...
      var instances = CACHE.GET('instances', 'name_'+instanceName, 'owner.github_'+info.ownerGithub);
      if (instances) {
        console.log('CACHEHIT', 'instances', 'name_'+instanceName, 'owner.github_'+info.ownerGithub);
        return _handleInstances();
      }
      console.log('CACHEMISS', 'instances', 'name_'+instanceName, 'owner.github_'+info.ownerGithub);
      instances = client.fetchInstances({
        name: instanceName,
        'owner.github': info.ownerGithub
      }, _handleInstances);
      function _handleInstances (err) {
        if (err) {
          log.error(put({
            err: err
          }, checkUrlLogData),
        ...
        CACHE.SET('instances', 'name_'+instanceName, 'owner.github_'+info.ownerGithub, instances);
...
api._handleElasticUrl = function (client, elasticUrl, refererUrl, masterInstance, cb) {
  ...
  function checkForAssociations(refererHostname, cb) {
    console.log('<checkForAssociations', refererHostname);
    ...
    findUserMapping(refererHostname, function (err, refererRoute) {
      ...
      var associations = CACHE.GET('association', elasticHostname, refererRoute.destInstanceId);
      if (associations) {
        console.log('CACHEHIT', 'association', elasticHostname, refererRoute.destInstanceId);
        return _handleAssoc(null, associations);
      }
      console.log('CACHEMISS', 'association', elasticHostname, refererRoute.destInstanceId);
      client
        .newInstance(refererRoute.destInstanceId)
        .fetchDependencies({
          hostname: elasticHostname
        }, _handleAssoc);
      function _handleAssoc (err, associations) {
        if (err) {
          log.error(put({
            err: err
          }, checkForAssociationsLogData),
        ...
        CACHE.SET('associations', elasticHostname, refererRoute.destInstanceId, associations);
        ...
        if (assoc) {
          ...
          var assocInstance = CACHE.GET('instance', assoc.id);
          if (assocInstance) {
            console.log('CACHEHIT', 'instance', assoc.id);
            return _handleInstance();
          }
          console.log('CACHEMISS', 'instance', assoc.id);
          assocInstance = client.fetchInstance(assoc.id, _handleInstance);
        }
        else {
        ...
        function _handleInstance (err) {
          if (err) {
            ...
            CACHE.SET('instance', assoc.id, assocInstance);
            log.trace(checkForAssociationsLogData,
                      'api._handleElasticUrl checkForAssociations findUserMapping '+
  ...
  function checkForMapping (noRouteHandler, cb) {
    console.log('<checkForMapping', (noRouteHandler || {}).name);
    ...
    findUserMapping(elasticHostname, function (err, route) {
      ...
      var mappedInstance = CACHE.GET('instance', route.destInstanceId);
      if (mappedInstance) {
        console.log('CACHEHIT', 'instance', route.destInstanceId);
        return _instanceHandler();
      }
      console.log('CACHEMISS', 'instance', route.destInstanceId);
      mappedInstance = client.fetchInstance(route.destInstanceId, _instanceHandler);
      function _instanceHandler (err) {
        if (err) {
          log.error(put({
            err: err
          ...
          CACHE.SET('instance', route.destInstanceId, mappedInstance);
  ...
  function findUserMapping (hostname, cb) {
    var token = client.client.opts.token;
    var route = CACHE.GET('findUserMapping', hostname, token);
    if (route) {
      console.log('CACHEHIT', 'findUserMapping', hostname, token);
      return cb(null, route);
    }
    console.log('CACHEMISS', 'findUserMapping', hostname, token);
    ...
    function handleRoutes (routes) {
      ...
      CACHE.SET('findUserMapping', hostname, token, route);
      cb(null, route);
    }
  }
  function masterUrl () {
...
api._handleUnauthenticated = function (req, res, next) {
  var reqUrl = api._getUrlFromRequest(req);
  req.redirectUrl = req.apiClient.getGithubAuthUrl(reqUrl, req.session.authTried);
  var logData = {
    tx: true,
    req: req,
