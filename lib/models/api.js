...
api.createClient = function (req, res, next) {
  ...
  if (req.method.toLowerCase() ===  'options' || !isBrowser) {
    req.apiClient = superUser;
    debug((!isBrowser ? 'non-browser' : 'options') + 'request, using super user');
    return next();
  }
 ...
 * @param  {handleDirectUrlCb}  cb  callback
 */
/**
 * @callback handleDirectUrlCb
 * @param {Error}  err
...
api._handleDirectUrl = function (urlEntry, client, instance, cb) {
  ...
  client.createRoute({
    ...
    if (err && /hello.*runnable/i.test(err.message)) {
      debug('hello runnable error', err);
      err = null;
    }
 ...
 * @param  {handleElasticUrlCb}  cb  callback
 */
/**
 * @callback handleElasticUrlCb
 * @param {Error}  err
 * @param {String} destHostname
 * @param {String} destInstanceName destination-instance's name
 */
api._handleElasticUrl = function (client, elasticUrl, refererUrl, masterInstance, cb) {
  ...
  function checkForAssociations(refererHostname, cb) {
    ...
    findUserMapping(refererHostname, function (err, refererRoute) {
      ...
      debug('Case: referer has a mapping, check associations', refererRoute);
      client
  ...
  function handleInstance (err, instance) {
    debug('err', err);
    if (err) { return cb(err); }
    instance.getContainerUrl(parsedElasticUrl.port, function (err, containerUrl) {
      cb(err, containerUrl, instance.attrs.name);
    });
  }
};
/**
  ...
  req.apiClient.fetch('me', function (err) {
    if (err) {
      ...
      if (isUnauthorizedErr(err)) {
        ...
        debug('user not logged in, url', url, 'authTried', req.session.authTried);
