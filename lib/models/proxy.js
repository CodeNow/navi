...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    var ownerGithubId = keypather.get(req, 'naviEntry.ownerGithubId');
    var isModerating = (keypather.get(req, 'headers.cookie') || '').indexOf('isModerating') > 0;
    if (ownerUsername && !isModerating) {
      var referer = req.header('Referrer') || '';
      var refererIsGithub = /github\.com/.test(referer);
      log.trace({
        referer: referer,
        refererIsGithub: refererIsGithub
      }, 'Referrer for request');
      monitor.increment('navi.hit', {
        org: ownerUsername.toLowerCase()
      });
      log.info({
        org: ownerUsername.toLowerCase()
      }, 'Navi Hit, excluding moderators');
      var updates = {
      ...
      log.trace({
        updates: updates
      }, 'Upserting user using Orion')
      orion.users.create(updates);
      rabbitMQ.publishApplicationUrlVisited({
        elasticUrl: req.elasticUrl,
        ownerGithubId: ownerGithubId,
        ownerUsername: ownerUsername,
        referer: req.header('Referrer'),
        refererIsGithub: refererIsGithub,
        shortHash: req.shortHash || 'not found',
        targetHost: targetHost
      });
    }
 ...
 * @param  {Boolean}  redirectEnabled  if redirection mode is enabled, we use this flag to
 * trigger script injection
 */
Proxy.prototype._streamRes = function (targetRes, proxiedRes, res, redirectEnabled) {
  ...
  if (resIsHtml && redirectEnabled) {
    log.trace(logData, '_streamRes resIsHtml and redirectEnabled');
    ...
    log.trace(logData, '_streamRes !resIsHtml || !redirectEnabled');
