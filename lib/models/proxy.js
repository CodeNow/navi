...
Proxy.prototype._addHeadersToRes = function (req, res, targetInstanceName) {
  ...
  var remoteAddr = this._getRemoteAddress(req);
  newHeaders['x-forwarded-for'] = remoteAddr;
  newHeaders['x-real-ip'] = remoteAddr;
  newHeaders['x-forwarded-protocol'] = req.isHttps ? 'https' : 'http';
  newHeaders['x-forwarded-proto'] = req.isHttps ? 'https' : 'http';
  newHeaders['x-forwarded-port'] = req.parsedReqUrl.port;
