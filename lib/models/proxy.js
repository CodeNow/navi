...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    self.proxy.on('start', function () {
      ...
      var remoteAddr = self._getRemoteAddress(req);
      var parsedTarget = url.parse(req.targetHost);
      req.headers['x-forwarded-for'] = remoteAddr;
      req.headers['x-real-ip'] = remoteAddr;
      req.headers['x-forwarded-protocol'] = parsedTarget.protocol;
      req.headers['x-forwarded-proto'] = parsedTarget.protocol;
      req.headers['x-forwarded-port'] = parsedTarget.port;
      log.trace({
       XXXX: req.headers
      }, 'XXX add headers');
    });
...
Proxy.prototype._getRemoteAddress = function (req) {
  var remoteAddr = keypather.get(req, 'connection.remoteAddress') ||
    keypather.get(req, 'connection.socket.remoteAddress');
  if (remoteAddr && remoteAddr.slice(0, 2) !== '::') {
