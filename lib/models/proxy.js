...
function Proxy () {
  ...
  this.proxy.on('error', function (err, req, res) {
    ...
    self.proxy.web(req, res, {
      target: errorPage.generateErrorUrl('unresponsive', req.targetInstance)
    });
  });
}
/**
...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    if (parsedTarget.query) {
      req.headers['x-runnable-query'] =  parsedTarget.query;
      debug('added headers', parsedTarget.query);
    }
    debug('host', host);
