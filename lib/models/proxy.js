...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    if (
      ownerUsername &&
      (keypather.get(req, 'headers.cookie') || '').indexOf('isModerating') === -1
    ) {
      var referrer = req.get('Referrer') || '';
      var refererIsGithub = referrer.match(/github\.com/) !== null;
      log.trace({
        referrer: referrer,
      ...
      if (refererIsGithub)  {
        updates.has_visited_navi_url_from_github = true;
