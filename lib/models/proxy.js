...
function Proxy () {
  ...
  this.proxy.on('error', function (err, req/*, proxiedRes */) {
    ...
    /**
     * Question, Anand:
     * I am not sure what the original intended purpose of this logic was. I think I should delete
     * the req.didError conditional check and set.
     */
    if (!req.didError) {
      req.didError = true;
...
Proxy.prototype._streamRes = function (targetRes, proxiedRes, res) {
  ...
  if (resIsHtml) {
    ...
    var stream = scriptInjectStream.output.pipe(res);
    stream.on('finish', function () {
      log.trace(logData, '_streamRes resIsHtml stream finish');
    });
    stream.on('error', function () {
      log.trace(logData, '_streamRes resIsHtml stream error');
    });
  }
  else {
    log.trace(logData, '_streamRes !resIsHtml');
