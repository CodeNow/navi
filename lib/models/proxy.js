...
function Proxy () {
  ...
  this.proxy.on('error', function (err, req, res) {
    ...
    req.targetInstance.elasticUrl = req.targetHost;
...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    // only set when we do not go to and error page
    console.log('XXXXX', req.targetInstance);
    if (req.targetInstance) {
      res.setHeader('X-Runnable-Branch', req.targetInstance.getBranchName());
      res.setHeader('X-Runnable-Id', req.targetInstance.id());
    }
