...
/**
 * responsible for proxying and redirecting request
 */
function Proxy () {
 ...
 * @param {String} req.redirectUrl  is required and must be a url
 * @return {function} middleware
 */
Proxy.proxyIfTargetHostExist = function (req, res, next) {
 ...
 * @return {function} middleware
 */
Proxy.send404 = function (req, res, next) {
  var redirectUrl = req.redirectUrl;
  debug('target redirectUrl', redirectUrl);
  // continue if we do not have a redirectUrl
  if (!redirectUrl) { return next(); }
