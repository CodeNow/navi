 ...
 */
Proxy.prototype.shouldUse = function (req) {
  var headers = keypath.get(req, 'headers.host');
  var session = keypath.get(req, 'session.userId');
  debug('headers', headers, 'session', session);
  if (typeof headers !== 'string'|| typeof session !== 'string') {
    return false;
  }
  return true;
};
...
Proxy.prototype.requestHandler = function () {
  ...
  return function (req, res, next) {
    ...
    Api.getHost(req, function (err, target) {
      ...
      // if we do not have a target next so we redirect
