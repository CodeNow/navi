...
function Proxy () {
  ...
  var self = this;
  if (this.shouldUseIntercom()) {
    this.intercomClient = new Intercom.Client(process.env.INTERCOM_APP_ID, process.env.INTERCOM_API_KEY);
  }
...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    if (ownerUsername && self.shouldUseIntercom()) {
      self.intercomClient.users.create({
        user_id: 'navi-' + ownerUsername,
        update_last_request_at: true,
        companies: [{
          id: ownerUsername.toLowerCase(),
          name: ownerUsername
        }]
      })
    }
