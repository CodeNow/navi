...
var patchXHRScriptStr = fs.readFileSync(
  path.join(__dirname, '')
);
...
Proxy.prototype.proxyIfTargetHostExist = function () {
  ...
  return function (req, res, next) {
    ...
    // This stream is created to buffer the response stream until
    // it is determined whether or not to transform the response
    var transformRes = through();

    self.proxy.web(req, transformRes, { target: targetHost });
    self.proxy.on('proxyRes', function (proxyRes) {
      ...
      if (proxyRes.headers['Content-Type'] === 'text/html') {
        // The response is transformed by injecting a script when content-type html is detected.
        transformRes = transformRes.pipe(scriptInjector(patchXHRScriptStr));
      }
      // if the response type is not html transformRes should not modify the response
      // finally pipe data to the real response
      transformRes.pipe(res);
    });
  };
};
/**
