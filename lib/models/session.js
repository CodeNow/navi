...
Session.getCookieFromToken = function (req, res, next) {
  ...
  redis.lpop(token, function (err, data) {
    ...
    if (data.sessionId) {
      log.trace(put({
        data: data
      }, logData), 'getCookieFromToken redis.lpop success data.sessionId');
      req.session.apiSessionId = data.sessionId;
    } else {
      log.trace(logData, 'getCookieFromToken redis.lpop success !data.sessionId');
    }
    next();
  });
};
