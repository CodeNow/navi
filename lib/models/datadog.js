...
function captureSocketCount () {
  ...
  for (key in sockets) {
    client.gauge('sisqo.sockets_open', sockets[key].length, 1,
      ['target:'+key, 'pid:'+process.pid]);
  }

  for (key in request) {
    client.gauge('sisqo.sockets_pending', request[key].length, 1,
      ['target:'+key, 'pid:'+process.pid]);
  }

  exec('lsof -p ' + process.pid + ' | wc -l', function (err, stdout) {
    if (err) { return; }
    client.gauge('sisqo.openFiles', parseInt(stdout), 1, ['pid:'+process.pid]);
  });
