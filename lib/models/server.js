...
function Server () {
  ...
  this.httpsServer = https.createServer({
    ca: fs.readFileSync(join(process.env.CERT_PATH, '/ca.pem')),
...
Server.prototype.start = function (cb) {
  ...
  mongo.start(function (err) {
    ...
    self.httpServer.listen(process.env.HTTP_PORT, function (err) {
      if (err) { return cb(err); }
      self.httpsServer.listen(process.env.HTTPS_PORT, cb);
    });
  });
};
...
Server.prototype._setupMiddleware = function () {
  ...
  this.httpsServer.on('upgrade', this._handleUserWsRequest());
};
