...
var resolveUrlsMiddleware = require('middlewares/resolve-urls');
var redirectDisabledMiddleware = require('middlewares/redirect-disabled');
var checkContainerStatusMiddleware = require('middlewares/check-container-status');
...
Server.prototype._setupMiddleware = function() {
  ...
  this.app.use(resolveUrlsMiddleware);
  this.app.use(redirectDisabledMiddleware);
  this.app.use(checkContainerStatusMiddleware);
  ...
  return function (req, socket, head) {
    ...
    _domain.run(function () {
      async.series([
        ...
        function (cb) {
          resolveUrlsMiddleware(req, null, cb);
        },
        function (cb) {
          redirectDisabledMiddleware(req, null, cb);
        },
        function (cb) {
          checkContainerStatusMiddleware(req, null, cb);
        },
        function (cb) {
          if (req.targetHost) {
